...
def main():
	...
	#Set up ColumnTransformer
		...
		model= TransformedTargetRegressor(Lasso(random_state = 0), func = np.log, inverse_func = np.exp)
...
# Looping through the outer loop, feeding each training set into a grid_search as the inner loop
	for train_index,test_index in outer_cv.split(X_train,y_train):
		...
		inner_loop_coefs.append(grid_search.best_estimator_.named_steps['lasso'].regressor_.coef_)
